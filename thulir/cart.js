function cartShower() {
  ...
  const displayCart = document.getElementById('cartActive');
  const fafa = document.getElementById('loginq');
  if (cart == null) {
    displayCart.style.opacity = '0';
    fafa.classList.remove('wobble'); // Hide the element
    // Show the element
  } else {
    displayCart.style.opacity = '1';
    fafa.classList.add('wobble'); // Hide the element
  }
...
function formAutoFill() {
  ...
  const dBase = localStorage.getItem('userDetails');
  const userDetails = JSON.parse(dBase);

  name.value = userDetails.name;
  address.value = userDetails.address;
  pincode.value = userDetails.pincode;
  contact.value = userDetails.contact;
}
...
function sendWhatsAppMessage() {
  const baseLink = 'https://api.whatsapp.com/send/?phone=%2B917695978577&text=';
...
function goInvo() {
  window.location.href = 'invo.html';
